...
class FA(nn.Module):
    def __init__(self, in_depth=1, in_channel=22, seq_len=640, modes=16, mode_select_method='segmented_random', dropout=0.3):
        super().__init__()
...
class FFT_Based_Refactor(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.k-1, 0, -1): topk[i] = topk[i-1]
        topk[0] = 1
        self.top_list = topk
        xf = torch.fft.rfft(x, dim=1)
...
class TPI(nn.Module):
    def __init__(self, F2, kernel_sizes=None, seq_len=40):
        super().__init__()
        self.seq_len = seq_len
        self.fft_ref = FFT_Based_Refactor(k=len(kernel_sizes) if kernel_sizes else 3)
        # 使用自定义kernel_sizes
    ...
    def forward(self, x):
        ...
        for i, p in enumerate(periods):
            ...
            out = self.conv(out)
            out = out.permute(0, 2, 3, 1).reshape(B, -1, N)[:, :self.seq_len, :]
            res.append(out)
...
class FACTNet(nn.Module):
    def __init__(self, nChan=22, nTime=640, nClass=4):
        ...
        self.use_fa = True
        if self.use_fa:
            self.fa = FA(in_depth=1, in_channel=nChan, seq_len=nTime, modes=16)
        self.temporal_conv = nn.Sequential(
            nn.Conv2d(F0, F1, (1,128), padding=(0,64), bias=False),
            nn.BatchNorm2d(F1)
        )
        self.channel_conv = nn.Sequential(
            Conv2dWithConstraint(F1, F1*D, (nChan,1), groups=F1, bias=False, max_norm=1.),
            nn.BatchNorm2d(F1*D), nn.ELU(), nn.AvgPool2d((1,4)), nn.Dropout(0.3)
        )
        self.depth_separable = nn.Sequential(
            nn.Conv2d(F1*D, F1*D, (1,16), padding=(0,8), groups=F1*D, bias=False),
            nn.Conv2d(F1*D, F2, (1,1), bias=False),
            nn.BatchNorm2d(F2), nn.ELU(), nn.AvgPool2d((1,4)), nn.Dropout(0.3)
        )
        self.depth_aware = EEGDepthAware(W=seq_len, C=F2, k=7)
        # 定义周期卷积核大小，如需修改，可传入更大值
        kernel_sizes = [15, 31, 63]
        self.model = nn.ModuleList([TPI(F2, kernel_sizes=kernel_sizes, seq_len=seq_len)])
        self.layer_norm = nn.LayerNorm(F2)
        self.classifier = nn.Sequential(
            nn.Flatten(), LinearWithConstraint(F2 * seq_len, nClass, max_norm=0.25), nn.Softmax(dim=-1)
        )
    def forward(self, x):
        if x.dim() != 4: x = x.unsqueeze(1)
        if self.use_fa: x = self.fa(x)
